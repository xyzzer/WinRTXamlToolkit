...
namespace WinRTXamlToolkit.Controls.AutoCompleteTextBoxControl.Algorithm
{
    public class CommonSubstringSuggestion : IAutoCompletable
    {
        ...
        {
            var suggestions = suggestionDictionary.Select(suggestion => GetScoreByLongestCommonSubstring(wordToSuggest, suggestion));
            int maximalScore = suggestions.Max(scoredString => scoredString.Score);

            return suggestions.Where(stringWithScore => stringWithScore.Score > 0 && maximalScore - stringWithScore.Score <= 1)
                              .OrderBy( scoreString => scoreString.Score, Comparer<int>.Create( (first, second) => second.CompareTo(first)) )
