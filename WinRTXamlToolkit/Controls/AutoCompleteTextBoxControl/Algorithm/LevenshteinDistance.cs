...
{
    public class DamerauLevenstheinDistance : IAutoCompletable
    {
        private readonly int maximalSuggestionCount = 25;
        ...
        {
            var scoredSuggestions = suggestionDictionary.Select(suggestion => CalculateDistance(wordToSuggest, suggestion));
            int maximalScore = scoredSuggestions.Min(scoredString => scoredString.Score); // less value means better

            IComparer<int> scoreComparer = Comparer<int>.Create( (firstScore, secondScore) => firstScore.CompareTo(secondScore) );
            return scoredSuggestions.Where(scoredString => scoredString.Score < wordToSuggest.Length && Math.Abs(maximalScore - scoredString.Score) <= 1)
                                    .OrderBy(scoredString => scoredString.Score, scoreComparer)
                                    .Take(maximalSuggestionCount)
