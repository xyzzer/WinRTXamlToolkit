...
namespace WinRTXamlToolkit.Controls.AutoCompleteTextBoxControl.Algorithm
{
    public class PrefixSuggestion : IAutoCompletable
    {
        ...
        {
            var scoredStrings = suggestionDictionary.Select(suggestion => GetSuggestionPrefixScore(wordToSuggest, suggestion));
            int maximalScore = scoredStrings.Max(scoredString => scoredString.Score);

            IComparer<int> scoreComparer = Comparer<int>.Create( (firstScore, secondScore) => secondScore.CompareTo(firstScore));
            return scoredStrings.Where(scoredString => scoredString.Score != 0 && maximalScore - scoredString.Score <= 1)
